    ...
    <section className={`glass-panel ${compact ? "p-5" : "min-h-[360px] p-5 sm:p-6"}`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-xl font-black text-brand-text-main">
            Protein / Fat / Carbs
          </h2>
        </div>
        <div className="rounded-2xl bg-brand-navy px-3 py-2 text-sm font900 text-white">
          {total}g
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {pieData.map((item, index) => (
          <div key={item.name} className="rounded-2xl bg-white/80 p-3 text-center ring-1 ring-brand-border">
            <div className="mx-auto h-2 w-9 rounded-full" style={{ backgroundColor: COLORS[index] }} />
            <div className="mt-2 text-lg font-black text-brand-text-main">
              {item.value}
              <span className="text-xs text-brand-text-sub">g</span>
            </div>
            <div className="text-xs font800 text-brand-text-sub">{item.name}</div>
          </div>
        ))}
      </div>
    </section>
...
function MacroTooltip({ active, payload }) {
  ...
  return (
    <div className="rounded-2xl border border-brand-border bg-brand-surface px-4 py-3 text-sm shadow-2xl shadow-brand-navy/10">
      <div className="font900 text-brand-text-main">{item.name}</div>
      <div className="font800 text-brand-primary">{item.value}{item.unit}</div>
    </div>
  );
}
